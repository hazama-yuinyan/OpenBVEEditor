...
ContextInformation::ContextInformation(const String& StrToAdd) : str(StrToAdd), layout(StrToAdd, Font(Font::getDefaultSansSerifFontName(), 16.0f, 0))
{
	layout.layout(300, Justification::left, false);
...
void ContextInformation::paint(Graphics& g)
{
	...
	layout.drawWithin(g, 0, 0, getWidth(), getHeight(), Justification::centred);
}
...
void AssistantContents::GetDocumentTypes(StringPairArray& Types)
{
	//You have to add file extensions and their corresponding type names.
	Types.set(L"Text", L".txt");
	Types.set(L"CSV", L".csv");
	Types.set(L"HTML", L".html");
	Types.set(L"Another version of HTML", L".htm");
}
