...
CodeTokeniser* MainComponent::CreateTokeniser(const String& FileExtension)
{
	...
	return (FileExtension == String(".csv")) ? new BVETokenizer(1000) :
		nullptr;
}
...
CodeAssistant* MainComponent::CreateAssistant(const String& FileExtension)
{
	...
	return (FileExtension == String(".csv")) ? new BVECodeAssistant() :
		nullptr;
}
...
void MainComponent::CreateFile(void)
{
	...
	*/
	...
	StringPairArray types;
	AssistantContents::GetDocumentTypes(types);
	...
	CodeTokeniser* tokeniser = CreateTokeniser(types.getValue(types.getAllKeys()[RESULT], String::empty));
	...
	AssistantWindow* assistant = new AssistantWindow(new AssistantContents(*editor_comp, CreateAssistant(types.getValue(types.getAllKeys()[RESULT], String::empty))));
