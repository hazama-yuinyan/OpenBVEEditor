...
void BVETokenizer::Init(void)
{
	...
	regexes[0] = bos >> L';' >> *~(set= L',');					//comment
	regexes[1] = bos >> (namespaces | icase(L"with") >> space >> namespaces);		//namespaces
	regexes[2] = commands >> before(space | L'(' >> _);				//commands
	regexes[3] = variable_names >> L'(' >> +digit >> L')' >> *(L'.' >> icase(L"set") | icase(L"load")) >> space >> +~(set= L',');	//variable
	regexes[4] = bos >> L'$' >> preprocess_names >> L'(' >> *digit >> L')';	//preprocess
	regexes[5] = bos >> +digit >> +(digit | L'.');	//position
}

void BVETokenizer::Tokenize(std::wstring& Str)
{
	results.clear();
	std::transform(regexes.begin(), regexes.end(), std::back_inserter(results), MakeRegexIterator(Str));
	results.erase(std::remove_if(results.begin(), results.end(), IsInvalid()), results.end());
	std::sort(results.begin(), results.end(), RegexCompare());
	result_it = begin();
	is_initialized = true;
}
...
{
	boost::xpressive::wsregex_id_filter_predicate comment_id(regexes[COMMENT-1].regex_id()), namespaces_id(regexes[NAMESPACE-1].regex_id()), command_id(regexes[COMMAND-1].regex_id())
...
int BVETokenizer::readNextToken(CodeDocument::Iterator& source)
{
	if(!is_initialized){
		...
		while(!source.isEOF() && source != line_end){
			content.push_back(static_cast<wchar_t>(source.peekNextChar()));		//行の終わり、または一つのexpressionの末端までの文字列をコピーする
			if(static_cast<wchar_t>(source.nextChar()) == L','){
				break;
			}
		}
...
const StringArray BVETokenizer::getTokenTypes(void)
{
	StringArray result;
